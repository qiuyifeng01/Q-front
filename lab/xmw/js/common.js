...
//新米分页  (当前页,总条数,每页显示条数,当前页前后显示页数)
function xmnextPage(pageIndex, total, pageSize, temp) {
    var ReturnHtml = "";
    pageIndex = parseInt(pageIndex);
    total = parseInt(total);
    pageSize = parseInt(pageSize);
    ...
    //计算当前位置
    ReturnHtml += "<div class='u-fr'>"

    //1    上一页 首页
    if (pageIndex == 1)
        ReturnHtml += "<a class='pa_disabled' title='上一页' href='javascript:void(0)'>上一页</a>";
    else
        ReturnHtml += "<a class='pa_disabled' title='上一页' k='" + (pageIndex - 1) + "' href='javascript:void(0)'>上一页</a><a href='javascript:void(0)' k='1' class='s-def'>1</a>";
    ...
    if (pageIndex + temp + 1 < pageCount)
        ...
        //7    尾页 下一页
    if (pageIndex == pageCount)
        ReturnHtml += "<a class='pg_enabled' title='下一页' href='javascript:void(0)'>下一页</a>";
    else
        ReturnHtml += "<a href='javascript:void(0)' k='"+pageCount+"' class='s-def'>"+pageCount+"</a><a class='pg_enabled' title='下一页' href='javascript:void(0)'  k='" + (pageIndex + 1) + "'>下一页</a>";
	
	ReturnHtml += "</div><div class='u-fr'><span class='u_fw'>" + total + "</span>条记录<span class='u_fw'>" + pageCount + "</span>页</div><div class='u-cls'></div>"
